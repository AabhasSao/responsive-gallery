...
// const collection = data.map(tile => {
//     let gall_item = document.createElement("div");
//     gall_item.setAttribute("class", "gallery-item");
//     let img_child = document.createElement("img");
//     img_child.setAttribute("src", tile.download_url);
//     img_child.setAttribute("alt", tile.author);
//     gall_item.appendChild(img_child);
//     return gall_item;
// })
...
for(let i = 0; i < data.length; i += 3) {
    let gall_row = document.createElement("div");
    gall_row.setAttribute("class", "gallery-row");
    ...
    for(let j = i; j < i + 3; j++) {
        ...
        let img_child = document.createElement("img");
        img_child.setAttribute("src", data[j].download_url);
        img_child.setAttribute("alt", data[j].author);
        gall_item.appendChild(post_container);
        post_container.appendChild(img_child);
        gall_row.appendChild(gall_item);
    }
    console.log(gall_row);
    grid.push(gall_row);
}

console.log(grid);

const gallery_ref = document.getElementById('gallery');
grid.map(e => gallery_ref.appendChild(e));
